...
# Load packages
pacman::p_load(haven, tidyverse, magrittr)
```
...
trets_returns = left_join(trets_df, mean_return_target)

# Calculate abnormal returns
trets_returns$abnorm = trets_returns$ret - trets_returns$mean_return
```
...
CAPM_returns = lapply(unique(trets_df$permno), function(x){
  trets_df |> 
    filter(rd <= -61) |> 
    filter(permno == {x}) %>%
    reframe(permno = permno, capm = coef(lm(ret ~ vwretd + -1,.))) |> # We're not using an intercept here because the variables already include the risk free rate
    unique()
}) |> bind_rows()

# Add capm returns back to data
trets_returns = left_join(trets_returns, CAPM_returns)
```
