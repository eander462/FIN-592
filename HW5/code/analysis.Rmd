...
In this question, I will rerun the analyis in question 2 but using equation (3). 

### a)

Table 6 shows the estimates from the regression. Table 7 shows the standard errors of a, and the $R^2$ values. 
...
Fama and French's patterns do hold up in the longer sample. First we can notice that there is still no spread in the $\beta$'s. The other two factors move in the same way they do in the Fama French paper. The $R^2$'s are similarly high. And finally there are several high t-statistics on the a's.

### c)

The GRS test for the three factor portfolio is `r GRS_3f` which has a p-value of `r GRS_3f_p_value`. The $\chi^2$ statistic is `r chi_3f` which has a p-value of `r chi_3f_p`. The mean absolute value of the $\alpha$'s is `r mean_abs_alpha_3f`.

\newpage

### d)

Below is a plot of the predicted vs the actual returns. As before, the blue line is the 45 degree line, the red points are the market returns, and the green point is the risk free asset.
...
Finally, I will compare the two models. The first thing to note is that both models are "rejected" by the GRS and $\chi^2$ tests. We easily reject the null that the $\alpha$'s are jointly 0. However, the 3 factor model has much smaller $\alpha$'s. The mean absolute $\alpha$ is `r mean_abs_alpha_3f` versus `r mean_abs_alpha` for the CAPM. So the pricing error is halved.

This tells us that reducing the testing of models to a single test statistic is overly simplistic. Despite having similar test statistics, the 3F model clearly explains more of the variation here. The graphs show it is a much better fit. As discussed in class, the 3 factor model shrinks the $\alpha$'s but also shrinks the variance-covariance matrix. These shrinking simultaneously means we end up with a similar test statistic because the smaller $\alpha$'s are measured better. 